...
//   PROPOSALS DO NOT COUNT. Bills, proposed rules, and "lawmakers are
//   considering" stories are dropped. Only signed/enacted/adopted/final items
//   with a real effective date make it in.
//   NEITHER DOES HISTORY. The calendar carries what is still ahead of us. A
//   change that is already in force is not a deadline, so it drops off.

const db = require("./db");
const { searchBatch, scrape } = require("./firecrawl");
const { STATES } = require("./states");
const { sourceTier, tierLabel, needsBetterSource, preferSource } = require("./sources");
...
// The calendar is about what is COMING, not what already happened. An item only
// lingers this many days past its effective date - just long enough for the
// "it is live now" reminder to go out - and then drops off.
const GRACE_DAYS = parseInt(process.env.LAW_GRACE_DAYS || "7", 10);
...
function buildJobs(tbs) {
  ...
  for (const state of STATES) {
    jobs.push({
      ...
      query: `"${state}" employment OR labor law changes taking effect employers new requirements ${year}`,
      opts: { tbs, limit: 5 }
    });
    jobs.push({
      key: `state:${state}:care`,
      query: `"${state}" home care OR Medicaid OR caregiver new law OR regulation signed takes effect ${year}`,
      opts: { tbs, limit: 5 }
    });
  }
  const fed = [
    `Department of Labor final rule upcoming effective date employer compliance deadline ${year}`,
    ...
    `OSHA OR EEOC OR NLRB OR I-9 final rule upcoming effective date employer requirements ${year}`,
    `CMS home health OR HCBS final rule upcoming compliance deadline effective date ${year}`
  ];
  // Primary-source lanes. Without these the sweep only sees whoever wrote
  // ABOUT a rule, which is how a federal Medicaid change got cited to a
  // competitor blog. These give the extractor a chance to cite the agency.
  const primary = [
    `site:federalregister.gov home care OR home health OR wage OR overtime effective date ${year}`,
    `site:cms.gov final rule home health OR HCBS OR Medicaid effective date ${year}`,
    `site:dol.gov final rule OR opinion letter employer effective date ${year}`,
    `site:medicaid.gov OR site:eeoc.gov OR site:osha.gov final rule effective date employers ${year}`
  ];
  primary.forEach((q, i) => jobs.push({ key: `primary:${i}`, query: q, opts: { tbs, limit: 6 } }));
...
function extractPrompt(results, existing) {
  ...
  return (
    ...
    `Your job is to maintain a forward-looking compliance calendar of LAW AND RULE CHANGES THAT ARE ALREADY LAW ` +
    `BUT HAVE NOT TAKEN EFFECT YET - what changed, where, when it kicks in, and what an employer / home care ` +
    `operator has to do before then.\n\n` +
    ...
    `TIMING - this is the most important filter after "is it actually law":\n` +
    `- The effective date MUST still be in the future. A change that took effect within the last ${GRACE_DAYS} days is ` +
    `the only exception.\n` +
    `- SKIP anything already in force. However important it was when it landed, it is history now, not a deadline.\n` +
    `- A change enacted months ago that does not take effect until later IS wanted. The enactment date can be old; ` +
    `the effective date cannot.\n` +
    `- Skip anything more than ${HORIZON_DAYS} days out.\n\n` +
    EXTRACT_RULES + `\n\n` +
...
async function verifyItem(item) {
  ...
  const prompt =
    ...
    `- If the page shows the change took effect more than ${GRACE_DAYS} days ago, return ` +
    `{"drop":true,"reason":"already in force"} - this calendar only carries what is still ahead.\n` +
...
// The forward-looking window: still ahead of us (or landed in the last few
// days), and near enough to be worth carrying.
...
//   whose effective dates are still ahead of us get picked up on the first run.
//   It widens how far back we look for news of an enactment, never which
//   effective dates qualify - already-in-force changes stay out either way.
...
async function sweep(opts = {}) {
  ...
  const existing = await db.listLawChanges({ openOnly: true, grace: GRACE_DAYS });
  ...
  items = items.filter(i => {
    ...
    if (!withinWindow(i)) { console.log(`[laws] dropped item outside the forward window: "${i.title}" (${i.effective_date})`); return false; }
    return true;
  });
  ...
  for (const item of items) {
    ...
    // A re-sweep must not downgrade a citation. If the tracked item already
    // points at the agency that made the rule, a later law-firm or press
    // write-up of the same change does not replace it.
    if (dupe && dupe.url) {
      const best = preferSource(dupe.url, record.url);
      if (best !== record.url) {
        console.log(`[laws] kept better source for "${record.title}": ${best} (over ${record.url})`);
        record = { ...record, url: best, source: dupe.source };
      } else if (best !== dupe.url) {
        console.log(`[laws] upgraded source for "${record.title}": ${best} (was ${dupe.url})`);
      }
    }
    try {
...
function toItem(row) {
  return {
    ...
    source_tier: sourceTier(row.url),
    source_tier_label: tierLabel(row.url),
    needs_better_source: needsBetterSource(row.url),
...
async function pending() {
  const rows = await db.listLawChanges({ openOnly: true, grace: GRACE_DAYS });
  const due = rows
    .filter(row => {
