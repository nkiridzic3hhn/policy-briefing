...
// Second pass for reputation items that are negative/needs-review OR fail the
// lookalike tripwire above: fetch the actual source page and rewrite the item
...
async function verifyFlaggedItems(items) {
  ...
  for (const item of items) {
    const flagged = ["negative", "review"].includes((item.sentiment || "").toLowerCase())
      || !brandMentioned(item);
    if (!flagged || !item.url || checked >= 6) { out.push(item); continue; }
    checked++;
    let page = "";
...
async function generateDigests(areas, days = LOOKBACK_DAYS) {
...
module.exports = { generateDigests, generatePolicy, generateFraud, generateReputation, LOOKBACK_DAYS };
