...
async function collectLaws(opts = {}) {
  ...
  // Two lists, deliberately: `due` is what crossed a milestone and earns the
  // full write-up, `calendar` is every open deadline. The email leads with the
  // former and lists the latter underneath, so a quiet week still shows the
  // reader everything they are on the hook for.
  const [due, calendar] = await Promise.all([lawtracker.pending(), lawtracker.calendar()]);
  return { due, calendar };
}

// Generate only the digests needed for a given set of areas.
// Shape: { policy: [], fraud: [], reputation: [], reputationQuiet: [], laws: [], lawsCalendar: [] }
...
async function generateDigests(areas, days = LOOKBACK_DAYS, opts = {}) {
  ...
  if (want.has("policy") && opts.laws !== false) {
    tasks.push(collectLaws(opts).then(r => { out.laws = r.due; out.lawsCalendar = r.calendar; }).catch(err => { console.error("[briefings] laws failed:", err.message); out.laws = []; out.lawsCalendar = []; }));
  }
