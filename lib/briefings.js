...
// criminal case is worse than no item at all.
async function verifyFlaggedItems(items) {
  ...
  for (const item of items) {
    ...
    if (!page || page.length < 120) {
      // Can't read the source (login walls, blocked bots). Publish only as an
      // unconfirmed lead, never as a factual claim.
      item.sentiment = "review";
      item.title = `Unverified mention of ${item.agency || "a watchlist brand"} — review the source`;
      item.summary = `A search snippet suggested: "${String(item.summary || "").slice(0, 180)}" — but the source page could not be read automatically, so treat this as unconfirmed and check the link directly.`;
      out.push(item);
      continue;
    }
    const prompt =
      ...
      `- Keep sentiment "negative" only if the page genuinely reflects negatively on the brand itself; use "review" if it is ambiguous.\n` +
      `Return ONLY raw JSON: {"drop": false, "title": "...", "summary": "1-2 sentences", "sentiment": "negative|review|neutral|positive|news"}`;
    try {
      ...
      console.error(`[briefings] verify pass failed: ${e.message}`);
      item.sentiment = "review";
    }
    out.push(item);
  }
