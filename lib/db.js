...
async function createInvite(email, name, token, days) {
  const res = await pool.query(
    `INSERT INTO invites (email, name, token, expires_at)
     VALUES ($1, $2, $3, now() + ($4 || ' days')::interval)
     RETURNING id, email, name, token, expires_at`,
    [String(email).toLowerCase().trim(), name || "", token, String(days || 14)]
  );
  return res.rows[0];
}
...
module.exports = {
  ...
  createInvite, findUsableInvite, consumeInvite, listInvites, revokeInvite
};
