...
async function init() {
  ...
  // Dashboard accounts. Before these existed the whole team shared one password
  // from an env var; that still works as a fallback so nothing breaks mid-migration.
  await pool.query(`
  ...
  `);
  // A single-use, expiring ticket to create one account. Kept separate from
  // users so an unaccepted invite is never a half-built login.
  await pool.query(`
  ...
  // Short typable code, added after the long-token links started tripping
  // corporate phishing filters. Nullable so invites issued before this exists
  // stay valid on their token.
  await pool.query(`ALTER TABLE invites ADD COLUMN IF NOT EXISTS code TEXT`);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS invites_code_idx ON invites (code) WHERE code IS NOT NULL`);
  await pool.query(`
...
async function createUser(email, name, passwordHash) {
  const res = await pool.query(
    ...
    [String(email).toLowerCase().trim(), name || "", passwordHash]
  );
  return res.rows[0];
}
...
async function listUsers() {
  const res = await pool.query(
    `SELECT id, email, name, status, created_at, last_login_at FROM users ORDER BY created_at DESC`
  );
  return res.rows;
}

async function deleteUser(id) {
  const res = await pool.query(`DELETE FROM users WHERE id = $1 RETURNING email`, [id]);
  return res.rows[0] || null;
}

// -- Invites --

async function createInvite(email, name, token, code, days) {
  const res = await pool.query(
    `INSERT INTO invites (email, name, token, code, expires_at)
     VALUES ($1, $2, $3, $4, now() + ($5 || ' days')::interval)
     RETURNING id, email, name, token, code, expires_at`,
    [String(email).toLowerCase().trim(), name || "", token, code || null, String(days || 14)]
  );
  return res.rows[0];
}

// Same usable-only rule as the token lookup: unused and unexpired, or nothing.
async function findUsableInviteByCode(code) {
  const c = String(code || "").toUpperCase().replace(/[^0-9A-Z]/g, "");
  if (!c) return null;
  const res = await pool.query(
    `SELECT * FROM invites WHERE code = $1 AND used_at IS NULL AND expires_at > now()`,
    [c]
  );
  return res.rows[0] || null;
}

// Only ever returns an invite that is still usable - unused and unexpired - so
// no caller can forget to check one of the two.
...
// Marks the invite used, but only if it still was: the UPDATE returns no row if
// another request got there first, which is what makes redeeming single-use
// even when the same link is opened twice at once.
async function consumeInvite(id) {
  const res = await pool.query(
    `UPDATE invites SET used_at = now() WHERE id = $1 AND used_at IS NULL RETURNING id`,
    [id]
  );
  return !!res.rows[0];
}
...
async function listInvites() {
  const res = await pool.query(
    `SELECT id, email, name, created_at, expires_at, used_at FROM invites ORDER BY created_at DESC LIMIT 50`
  );
  return res.rows;
}

async function revokeInvite(id) {
  const res = await pool.query(`DELETE FROM invites WHERE id = $1 RETURNING email`, [id]);
  return res.rows[0] || null;
}
...
module.exports = {
  ...
  findUserByEmail, createUser, touchUserLogin, listUsers, deleteUser,
  createInvite, findUsableInvite, findUsableInviteByCode, consumeInvite, listInvites, revokeInvite
};
